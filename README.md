...
- **总结接口（小白x / SP·数据库）**：玩家面板"总结接口"页签可启用小白x剧情总结或 SP·数据库 表格总结——分析时已总结楼层用其总结内容替代原文注入，总结边界之后的新楼层照常读取原文；周期位置、楼层计数、任务书签计算完全不受影响；未安装或无数据时自动回退纯原文模式。小白x 读聊天元数据里的结构化总结（事件/人物事实/剧情阶段）；SP·数据库 读其写入世界书的可读条目（总结表/重要人物/剧情大纲，按消息上的 TavernDB 表格快照判定边界）——均只读、零模块耦合、不修改插件数据；随配置模板导出/导入
